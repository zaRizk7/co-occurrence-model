...
switch dataset
 ...
 case 16
    name = 'coco2017';
    if ~exist('coco2017-cooccurences-train.csv', 'file')
         error(msg);
    end
    data = readtable('coco2017-cooccurences-train.csv');
    features = data(:, 9:end);

    for i = 1:length(features.Properties.VariableNames),
        col = split(features.Properties.VariableNames{i}, '_');
        col = col{2};
        features.Properties.VariableNames{i} = col;
    end

    N = height(features);
    K = max(features, [], 'all').max(1);
    K = K + 1;

    dtr.name = name;
    dtr.names = features.Properties.VariableNames;
    dtr.x = transpose(cast(table2array(features), 'int32')) + 1;
    dtr.nsyms = zeros(1, 80, 'int16') + K;
    size(dtr.nsyms)

    if ~exist('coco2017-cooccurences-valid.csv', 'file')
         error(msg);
    end
    data = readtable('coco2017-cooccurences-valid.csv');
    features = data(:, 9:end);
    ...
    dte.name = name;
    dte.names = features.Properties.VariableNames;
    dte.x = transpose(cast(table2array(features), 'int32')) + 1;
    dte.nsyms = dtr.nsyms;
